...
#define CHAR_DEV_NAME "sunny_cdrv"
#define MAX_LENGTH 4000
#define SUCCESS 0

static char *char_device_buf;
struct cdev *sunny_cdev;
dev_t mydev;
int count=1;
static struct class *sunny_class;
...
static __init int char_dev_init(void)
{
	...
	if (alloc_chrdev_region (&mydev, 0, count, CHAR_DEV_NAME) < 0) {
        ...
        if (!(sunny_cdev = cdev_alloc ())) {
            printk (KERN_ERR "cdev_alloc() failed\n");
            unregister_chrdev_region (mydev, count);
            return -1;
 	}
	cdev_init(sunny_cdev,&char_dev_fops);

	ret=cdev_add(sunny_cdev,mydev,count);
	if( ret < 0 ) {
...
{
	 device_destroy (sunny_class, mydev);
         class_destroy (sunny_class);
	 cdev_del(sunny_cdev);
